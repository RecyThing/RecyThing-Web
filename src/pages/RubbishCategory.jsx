...
function RubbishCategory() {
	...
	const { status: updateStatus, message: updateMessage } = useSelector(updateTrashesSelector);
	const { status: deleteStatus, message: deleteMessage } = useSelector(deleteTrashesSelector);
	const { status: createStatus, message: createMessage } = useSelector(createTrashesSelector);
	...
	const filteredData = Object.values(data).filter((trashes) => {
		return trashes.trash_type && trashes.trash_type.toLowerCase().includes(searchTerm.toLowerCase());
	});
