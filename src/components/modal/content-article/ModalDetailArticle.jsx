	...
	return (
		...
		>
			...
			>
				...
				<img
					src={currArticleData.image}
					className="my-8 object-cover h-[148px]"
					alt=""
				/>
				<div className="flex justify-between">
					<div className="flex gap-2">
						<p className="text-sm overflow-hidden text-ellipsis whitespace-nowrap max-w-[270px] text-[#35CC33]">{currArticleData.categories?.map((item) => item.category).join(" ")}</p>
				...
				<div className="mt-2 max-h-[353px] overflow-y-scroll">{Parse(currArticleData.content || "")}</div>
				...
				<div className="mt-8 flex gap-3 justify-between text-white">
					...
					<button
						...
						className="p-4 w-full rounded-lg bg-[#35CC33] hover:opacity-90"
					>
						Ubah
					</button>
				</div>
