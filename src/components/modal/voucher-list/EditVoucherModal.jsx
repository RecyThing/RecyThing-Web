...
export function EditVoucherModal({ isOpen, onClose, target, onSubmit }) {
	const {
		handleSubmit,
		control,
		formState: { errors },
	...
	useEffect(() => {
		if (target) {
			setValue("voucherImage", target.voucherImage);
			setValue("voucherName", target.voucherName);
			setValue("voucherPoint", target.voucherPoint);
			setValue("voucherDescription", target.voucherDescription);
			setValue("voucherStartDate", target.voucherStartDate);
			setValue("voucherEndDate", target.voucherEndDate);
		}
	}, [target, setValue]);
