...
const dataContent = {
    question: "1. Apa saja contoh sampah plastik?\n2. Bagaimana mengelola sampah plastik? Mengapa sampah plastik dapat mencemari lingkungan?\n3. Apa dampak yang ditimbulkan jika kita tidak mengelola sampah plastik dengan baik?\n4. Bagaimana cara mendaur ulang sampah plastik?",
};

export function EditDataCustomizationModal({ isOpen, onClose, setIsEditData }){
    const [question, setQuestion] = useState(dataContent.question);
    ...
    const closeForm = () => {
        setIsEditData(false);
        setQuestion(dataContent.question);
        onClose();
    };
                ...
                <ModalHeader className="flex justify-between mt-8">
                    ...
                    <form onSubmit={onSubmit}>
                        <div className="w-72 border rounded-xl p-3 mb-6 relative" style={{ borderColor: "rgba(130, 130, 130, 1)"}}>
                            <select name="topik" id="topik" className="w-64 outline-none text-sm border-none bg-transparent" style={{ color: "rgba(79, 79, 79, 1)" }}>
                        ...
                        <div className="w-full border rounded-xl p-3 relative" style={{ borderColor: "rgba(130, 130, 130, 1)"}}>
                            <textarea name="question" id="question" className="w-full h-36 outline-none text-sm border-none bg-transparent pt-4" style={{ color: "#4F4F4F" }} placeholder="Masukkan question" value={question} onChange={(e) => setQuestion(e.target.value)}></textarea>
                            <label htmlFor="question" className="text-xs absolute -top-2 left-3 bg-white px-1" style={{color: "rgba(130, 130, 130, 1)"}}>question</label>
                        </div>
                    </form>
                </ModalBody>
