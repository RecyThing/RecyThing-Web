  ...
  if (sideBarCollapse) return (
    <div onClick={handleClickMenu} className={`cursor-pointer p-3 rounded-lg ${(pathname === path || isSubMenuSelected) ?
     'bg-[#35CC33] text-white' : 'hover:bg-slate-100'}`}>
      {name === "Transaksi Drop Point" ? <img src={(pathname === path || isSubMenuSelected) ? recycleWhite : recycle} 
      alt="" className="shrink-0 w-6 h-6" /> : logo}
    </div>
  )
  ...
  return (
    <div className="cursor-pointer min-w-[263px]">
      ...
      ${(pathname === path || isSubMenuSelected) ? 'bg-[#35CC33] text-white' : 'hover:bg-slate-100'}`}>
        ...
        {!path ? expandedMenu === name ? <PiCaretUp className="my-auto" /> : <PiCaretDown className="my-auto" /> : <></> }
      </div>
 
      <div className={`overflow-hidden transition-all duration-[350ms] ${(!path && expandedMenu === name) ?
